...
def test_all_case(net, ema_net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None):
    total_metric = 0.0
    cnt = 0
    ...
    for image_path in tqdm(image_list):
        ...
        prediction, prediction_fusion, prediction_conf8, prediction_conf7, prediction_conf6, prediction_conf5, sample_entropy_single, confidence_single, diversity_single, edgeInformation_single = test_single_case(cnt, net, ema_net, image, stride_xy, stride_z, patch_size, num_classes=num_classes, test_save_path=test_save_path)
...
def test_single_case(cnt_epoch, net, ema_net, image, stride_xy, stride_z, patch_size,test_save_path, num_classes=1):
    w, h, d = image.shape
    ...
    score_map_ema = np.zeros((num_classes, ) + image.shape).astype(np.float32)
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                noise = torch.clamp(torch.randn_like(test_patch) * 0.1, -0.2, 0.2)
                ema_test_patch = test_patch + noise
                y1_dis, y1, y1_conf8, y1_conf7, y1_conf6, y1_conf5 = net(test_patch)
                _, y1_ema, _, _, _, _ = ema_net(ema_test_patch)
                ...
                score_map_ema[:, xs:xs+patch_size[0], ys:ys+patch_size[1], zs:zs+patch_size[2]] \
                  = score_map_ema[:, xs:xs+patch_size[0], ys:ys+patch_size[1], zs:zs+patch_size[2]] + y_ema
