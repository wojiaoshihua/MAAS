...
parser.add_argument('--exp', type=str,  default='UAMT_unlabel16', help='model_name')
...
if __name__ == "__main__":
    ...
    save_mode_path2 = "/home/zhangmingxiu/UA-MT-master/model/UAMT_unlabel8/UAMT(multi_fusion)/iter_6400_dice_0.8759606902476845.pth"
    model.load_state_dict(torch.load(save_mode_path2))
    save_mode_path2 = "/home/zhangmingxiu/UA-MT-master/model/UAMT_unlabel8/UAMT(multi_fusion)/ema_iter_6400_dice_0.8759606902476845.pth"
...
#cross_task_dist = torch.sum(mask*torch.mean((dis_to_mask - outputs_soft) ** 2))/(2*torch.sum(mask)+1e-16)      
            ...
            if iter_num % 100 == 0:
                ...
                if dice_sample > best_dice:
                    ...
                    save_best_path = os.path.join(snapshot_path,'./UAMT(multi_fusion4816)/my_best_model.pth')
                    save_bestema_path = os.path.join(snapshot_path,'./UAMT(multi_fusion4816)/my_best_emamodel.pth')
                    torch.save(model.state_dict(), save_best_path)
                    torch.save(ema_model.state_dict(), save_bestema_path)
                    logging.info("save best model to {}".format(save_best_path))
                save_mode_path = os.path.join(snapshot_path,  './UAMT(multi_fusion4816)/iter_{}_dice_{}.pth'.format(iter_num, dice_sample))
                save_modeema_path = os.path.join(snapshot_path,  './UAMT(multi_fusion4816)/ema_iter_{}_dice_{}.pth'.format(iter_num, dice_sample))
                torch.save(model.state_dict(), save_mode_path)
        ...
        if iter_num >= max_iterations:
            break
    save_mode_path = os.path.join(snapshot_path, './UAMT(multi_fusion4816)/iter_'+str(max_iterations)+'.pth')
    save_modeema_path = os.path.join(snapshot_path, './UAMT(multi_fusion4816)/iter_'+str(max_iterations)+'ema.pth')
    torch.save(model.state_dict(), save_mode_path)
    torch.save(ema_model.state_dict(), save_modeema_path)
    logging.info("save model to {}".format(save_mode_path))
    writer.close()
