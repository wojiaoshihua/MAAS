...
class Decoder(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False, up_type=0):
        ...
        if up_type == 1:
            self.conf5 = nn.Sequential(nn.Conv3d(n_filters * 16, n_classes, kernel_size=1, padding=0),
                                        nn.Upsample(scale_factor=16, mode="trilinear", align_corners=False), )
            self.conf6 = nn.Sequential(nn.Conv3d(n_filters * 8, n_classes, kernel_size=1, padding=0),
                                        nn.Upsample(scale_factor=8, mode="trilinear", align_corners=False), )
            self.conf7 = nn.Sequential(nn.Conv3d(n_filters * 4, n_classes, kernel_size=1, padding=0),
                                        nn.Upsample(scale_factor=4, mode="trilinear", align_corners=False), )
            self.conf8 = nn.Sequential(nn.Conv3d(n_filters * 2, n_classes, kernel_size=1, padding=0),
    ...
    def forward(self, features):
        ...
        if self.up_type == 1:
            x5_conf = self.conf5(x5)
        x5_up = self.block_five_up(x5)
        x5_up = x5_up + x4

        x6 = self.block_six(x5_up)
        if self.up_type == 1:
            x6_conf = self.conf6(x6)
        x6_up = self.block_six_up(x6)
        x6_up = x6_up + x3

        x7 = self.block_seven(x6_up)
        if self.up_type == 1:
            x7_conf = self.conf7(x7)
        x7_up = self.block_seven_up(x7)
        x7_up = x7_up + x2

        x8 = self.block_eight(x7_up)
        if self.up_type == 1:
            x8_conf = self.conf8(x8)
        ...
        ###############
        out_tanh = self.tanh(out_seg)
        
        if self.up_type == 1:
            return out_tanh, out_seg, x8_conf, x7_conf, x6_conf, x5_conf
...
class VNet(nn.Module):
    ...
    def forward(self, input):
        ...
        out_dis2, out_seg2, x8_conf, x7_conf, x6_conf, x5_conf = self.decoder1(features)
        return out_seg2, out_dis2, x8_conf, x7_conf, x6_conf, x5_conf
...
class MCNet3d_v1(nn.Module):
    ...
    def forward(self, input):
        features = self.encoder(input)
        out_dis1, out_seg1 = self.decoder1(features)
        out_dis2, out_seg2 = self.decoder2(features)
        return out_seg1, out_seg2, out_dis1, out_dis2
...
class MCNet3d_v2(nn.Module):
    ...
    def forward(self, input):
        features = self.encoder(input)
        out_dis1, out_seg1 = self.decoder1(features)
        out_dis2, out_seg2, x8_conf, x7_conf, x6_conf, x5_conf = self.decoder2(features)
        out_dis3, out_seg3 = self.decoder3(features)
        return out_seg1, out_seg2, out_seg3, out_dis1, out_dis2, out_dis3, x8_conf, x7_conf, x6_conf, x5_conf
