...
class unet_3D_dt(nn.Module):
    ...
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3, is_batchnorm=True):
        ...
        self.conf5 = UnetDsv3(in_size=filters[3], out_size=n_classes, scale_factor=8)
        self.conf6 = UnetDsv3(in_size=filters[2], out_size=n_classes, scale_factor=4)
        self.conf7 = UnetDsv3(in_size=filters[1], out_size=n_classes, scale_factor=2)
        #self.conf8 = nn.Conv3d(in_channels=filters[0], out_channels=n_classes, kernel_size=1)
        self.conf8 = nn.Conv3d(filters[0], n_classes, 1)
        # initialise weights
        for m in self.modules():
            if isinstance(m, nn.Conv3d):
                init_weights(m, init_type='kaiming')
    ...
    def forward(self, inputs):
        ...
        x5_conf5 = self.conf5(up4)
        x6_conf6 = self.conf6(up3)
        x7_conf7 = self.conf7(up2)
        x8_conf8 = self.conf8(up1)
        return dis,final,x8_conf8,x7_conf7,x6_conf6,x5_conf5
