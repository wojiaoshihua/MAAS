...
def test_single_case(net, image, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                with torch.no_grad():
                    dis, y, y_conf5, y_conf6, y_conf7, y_conf8 = net(test_patch)
                    y = torch.softmax(y, dim=1)
                    y_conf8 = F.softmax(y_conf8, dim=1)
                    y_conf7 = F.softmax(y_conf7, dim=1)
                    y_conf6 = F.softmax(y_conf6, dim=1)
                    y_conf5 = F.softmax(y_conf5, dim=1)
                    y_dis = F.softmax(-1500*dis, dim=1)
    ...
    #label_map_conf8 = (score_map_conf8[0]>0.5).astype(np.int)
...
def test_all_case(net, base_dir, method="unet_3D", test_list="full_test.list", num_classes=4, patch_size=(48, 160, 160), stride_xy=32, stride_z=24, test_save_path=None):
    ...
    image_list = [base_dir + "/h5data/{}.h5".format(
    ...
    with open(test_save_path + "/{}.txt".format(method), "a") as f:
        for image_path in tqdm(image_list):
            ...
            image = h5f['image'][:]
            label = h5f['label'][:]
            prediction, score_map, prediction_conf8, prediction_conf7, prediction_conf6, prediction_conf5, sample_entropy_single, confidence_single, diversity_single, edgeInformation_single = test_single_case(
            ...
            sitk.WriteImage(pred_itk, test_save_path +
                            "/entropy"+str(a)+"_{}.nii.gz".format(ids))
            sitk.WriteImage(pred_itk, test_save_path +
                            "/confidence"+str(b)+"_{}.nii.gz".format(ids))
            sitk.WriteImage(pred_itk, test_save_path +
                            "/diversity"+str(c)+"_{}.nii.gz".format(ids))
            sitk.WriteImage(pred_itk, test_save_path +
                            "/edg"+str(d)+"e_{}.nii.gz".format(ids))
            pred_itk = sitk.GetImageFromArray(prediction_conf8.astype(np.uint8))
            pred_itk.SetSpacing((1.0, 1.0, 1.0))
            sitk.WriteImage(pred_itk, test_save_path +
                            "/{}_conf8.nii.gz".format(ids))
            pred_itk = sitk.GetImageFromArray(prediction_conf7.astype(np.uint8))
            pred_itk.SetSpacing((1.0, 1.0, 1.0))
            sitk.WriteImage(pred_itk, test_save_path +
                            "/{}_conf7.nii.gz".format(ids))
            pred_itk = sitk.GetImageFromArray(prediction_conf6.astype(np.uint8))
            pred_itk.SetSpacing((1.0, 1.0, 1.0))
            sitk.WriteImage(pred_itk, test_save_path +
                            "/{}_conf6.nii.gz".format(ids))
            pred_itk = sitk.GetImageFromArray(prediction_conf5.astype(np.uint8))
            pred_itk.SetSpacing((1.0, 1.0, 1.0))
            sitk.WriteImage(pred_itk, test_save_path +
                            "/{}_conf5.nii.gz".format(ids))
