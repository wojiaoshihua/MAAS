...
parser.add_argument('--root_path', type=str,
                    default='../../dataset/2018LA_Seg_Training Set', help='Name of Experiment')
...
def train(args, snapshot_path):
    ...
    save_mode_path2 = "/home/zhangmingxiu/AC-MT-main/model/LA_ACMT_SErr_clv2_8/vnet/multi_initial/iter_9400_dice_0.8969.pth"
    model.load_state_dict(torch.load(save_mode_path2))
    save_mode_path2 = "/home/zhangmingxiu/AC-MT-main/model/LA_ACMT_SErr_clv2_8/vnet/multi_initial/emaiter_9400_dice_0.8969.pth"
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            out_conf8_soft = torch.softmax(out_conf8, dim=1)
            out_conf7_soft = torch.softmax(out_conf7, dim=1)
            out_conf6_soft = torch.softmax(out_conf6, dim=1)
            out_conf5_soft = torch.softmax(out_conf5, dim=1)
            outputs_onehot = torch.argmax(outputs_soft, dim=1)
            with torch.no_grad():
                ema_outdis, ema_output, emaout_conf8, emaout_conf7, emaout_conf6, emaout_conf5 = ema_model(ema_inputs)
                ema_output_soft = torch.softmax(ema_output, dim=1)
                noisy_ema_outdis, noisy_ema_output, noisy_emaout_conf8, noisy_emaout_conf7, noisy_emaout_conf6, noisy_emaout_conf5 = ema_model(noisy_ema_inputs)
            ...
            consistency_dist_conf8 = losses.softmax_mse_loss(outputs[args.labeled_bs:], noisy_emaout_conf8)
            consistency_dist_conf7 = losses.softmax_mse_loss(outputs[args.labeled_bs:], noisy_emaout_conf7)
            consistency_dist_conf6 = losses.softmax_mse_loss(outputs[args.labeled_bs:], noisy_emaout_conf6)
            consistency_dist_conf5 = losses.softmax_mse_loss(outputs[args.labeled_bs:], noisy_emaout_conf5)
            ...
            try:
                ...
                consistency_loss_conf8 = torch.sum(mask*consistency_dist_conf8)/(torch.sum(mask)+1e-16)
                consistency_loss_conf7 = torch.sum(mask*consistency_dist_conf7)/(torch.sum(mask)+1e-16)
                consistency_loss_conf6 = torch.sum(mask*consistency_dist_conf6)/(torch.sum(mask)+1e-16)
                consistency_loss_conf5 = torch.sum(mask*consistency_dist_conf5)/(torch.sum(mask)+1e-16)
            ...
            loss_ce_conf8 = ce_loss(out_conf8[:args.labeled_bs], label_batch[:args.labeled_bs][:])
            loss_ce_conf7 = ce_loss(out_conf7[:args.labeled_bs], label_batch[:args.labeled_bs][:])
            loss_ce_conf6 = ce_loss(out_conf6[:args.labeled_bs], label_batch[:args.labeled_bs][:])
            loss_ce_conf5 = ce_loss(out_conf5[:args.labeled_bs], label_batch[:args.labeled_bs][:])
            loss_dice = dice_loss(outputs_soft[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1))
            loss_dice_conf8 = dice_loss(out_conf8_soft[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1))
            loss_dice_conf7 = dice_loss(out_conf7_soft[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1))
            loss_dice_conf6 = dice_loss(out_conf6_soft[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1))
            loss_dice_conf5 = dice_loss(out_conf5_soft[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1))
            
            supervised_loss = 0.5 * (loss_dice + loss_dice_conf5 + loss_dice_conf6 + loss_dice_conf7 + loss_dice_conf8
             + loss_ce_conf8 + loss_ce_conf7 + loss_ce_conf6 + loss_ce_conf5 + loss_ce)
            loss = supervised_loss + consistency_weight * (consistency_loss + consistency_loss_conf8 + consistency_loss_conf7 + consistency_loss_conf6 + consistency_loss_conf5)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
#####
            ...
            if iter_num % 100 == 0:
                ...
                if avg_metric[:, 0].mean() > best_performance:
                    ...
                    save_modeema_path = os.path.join(snapshot_path,
                                                  'emaiter_{}_dice_{}.pth'.format(
                                                      iter_num, round(best_performance, 4)))
...
if __name__ == "__main__":
    ...
    snapshot_path = "../model/{}_{}/{}/multi_initial".format(
